...
def top_n_extract_show(file2, topN_number):
    ...
    fig = plt.gcf()  # get current figure 도화지를 먼저 그린다.
    fig.set_size_inches(20, 10)
    matplotlib.rc("font", family="AppleGothic", size=10)
    plt.title("그래프 제목")
    plt.xlabel("단어")
    plt.ylabel("개수")
...
def wordcloud(file1):
    with open(file1, "r", encoding="utf-8") as f:
        ...
        if not tags:  # tags가 비어있는 경우
            print("There are no tags to generate a word cloud.")
            return

        wc = WordCloud(font_path="/System/Library/Fonts/AppleSDGothicNeo.ttc", background_color=(168, 237, 244),
